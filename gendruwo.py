...
def bacaData():
	...
	try:
		fid_bgroup = open(os.sys.path[0]+'/gendruwobgroup.txt','r').readlines()
	except:pass
	try:
		fid_bteman = open(os.sys.path[0]+'/gendruwobteman.txt','r').readlines()
	except:pass
def inputD(x,v=0):
	while 1:
		try:
...
def simpan():
	if len(id_bgroup) != 0:
		...
		try:
			open(os.sys.path[0]+'/gendruwobgroup.txt','w').write('\n'.join(id_bgroup))
			tampil('\rh[!]Berhasil meyimpan \rcgendruwobgroup.txt')
		except:
			tampil('\rm[!]Gagal meyimpan')
	if len(id_bteman) != 0:
		tampil('\rh[*]Menyimpan hasil daftar Teman...')
		try:
			open(os.sys.path[0]+'/gendruwobteman.txt','w').write('\n'.join(id_bteman))
			tampil('\rh[!]Berhasil meyimpan \rcgendruwobgteman.txt')
		except:
			tampil('\rm[!]Gagal meyimpan')
def buka(d):
...
def lanjutT():
	...
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/gendruwobteman.txt')
			fid_bteman = []
	return 0
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/gendruwobgroup.txt')
			fid_bgroup = []
	return 0
def menu():
	tampil('''\rh
            ...
            \rk*   *\rh /+/++//
             \ /  |/__//
           {\rmX\rh}v{\rmX\rh}|\rc=====\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_    \rk*\godeyes
\rk###########################################################
#             \rb*GENDRUWO   FACEBOOK*\rk                 #
# \rhBY\rp                                             godeyes \rk#
# \rhGroup FB\rp  https://m.facebook.com/groups/168751979837 \rk#
# \rhGitHub\rp                      https://github.com/godeyes\rk#
#     \rhGitHub\rp                  https://github.com/santai \rk#
###########################################################''')
